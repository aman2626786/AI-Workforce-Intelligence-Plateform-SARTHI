...
def parse_file(filepath, resource_type, existing_slugs):
    ...
    for i, m in enumerate(matches):
        ...
        item = {
            ...
            "view_count": 85 + (len(existing_slugs) * 11) % 450,
            "like_count": 18 + (len(existing_slugs) * 5) % 85,
            "save_count": 12 + (len(existing_slugs) * 3) % 65,
            "share_count": 6 + len(existing_slugs) % 30,
            "comment_count": 2 + len(existing_slugs) % 8,
            "status": "PUBLISHED",
