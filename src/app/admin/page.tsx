        ...
        {activeSection === 'resource_editor' && (
          <div className="space-y-6 animate-fade-in">
            ...
            <form onSubmit={handlePublishResourceOrNews} className="space-y-6">
              ...
              {/* ========================================================================= */}
              {/* PLATFORM RESOURCE & PRACTICE LINKS WITH UPLOADED LOGOS (TASK 2 & TASK 3) */}
              {/* ========================================================================= */}
              <div className="p-6 rounded-3xl bg-white/95 border-2 border-sky-200/80 shadow-sm space-y-5">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-sky-100 pb-3">
                  <div>
                    <h4 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                      <span>Practice & Code Verification Links (5 Supported Platforms)</span>
                      <span className="px-2 py-0.5 rounded text-[10px] bg-sky-100 text-sky-800 border border-sky-200 font-black">
                        {attachedLinks.length} Active
                      </span>
                    </h4>
                    <p className="text-[11px] text-slate-600">
                      Attach verified practice links with official branded logos (LeetCode, Hugging Face, Kaggle, GitHub, Official Docs) for students to practice directly.
                    </p>
                  </div>
                </div>

                {/* Add New Link Control Box (Task 2: Strictly the 5 Allowed Platforms) */}
                <div className="p-4 rounded-2xl bg-sky-50/60 border-2 border-sky-200/80 space-y-3">
                  <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                    {/* Platform Selector with 5 Allowed Options */}
                    <div className="space-y-1">
                      <label className="text-[11px] font-extrabold text-slate-700">Choose Platform</label>
                      <select
                        value={newLinkPlatform}
                        onChange={(e) => setNewLinkPlatform(e.target.value)}
                        className="w-full px-3 py-2.5 bg-white border-2 border-sky-200 rounded-xl text-xs text-slate-900 font-bold focus:outline-none focus:ring-2 focus:ring-sky-500 shadow-2xs"
                      >
                        <option value="leetcode">💡 LeetCode Practice</option>
                        <option value="huggingface">🤗 Hugging Face Hub</option>
                        <option value="kaggle">📊 Kaggle Platform</option>
                        <option value="github">🐙 GitHub Repository</option>
                        <option value="web">🌐 Web Browser / Official Docs</option>
                      </select>
                    </div>

                    {/* Link Label */}
                    <div className="sm:col-span-1 space-y-1">
                      <label className="text-[11px] font-extrabold text-slate-700">Link Label / Title</label>
                      <input
                        type="text"
                        value={newLinkTitle}
                        onChange={(e) => setNewLinkTitle(e.target.value)}
                        placeholder="e.g. Practice LeetCode Problem"
                        className="w-full px-3 py-2.5 bg-white border-2 border-sky-200 rounded-xl text-xs text-slate-900 font-medium focus:outline-none focus:ring-2 focus:ring-sky-500 shadow-2xs"
                      />
                    </div>

                    {/* Destination URL */}
                    <div className="sm:col-span-2 space-y-1">
                      <label className="text-[11px] font-extrabold text-slate-700">Destination URL</label>
                      <div className="flex items-center gap-2">
                        <input
                          type="url"
                          value={newLinkUrl}
                          onChange={(e) => setNewLinkUrl(e.target.value)}
                          placeholder="https://leetcode.com/... or https://github.com/..."
                          className="w-full px-3 py-2.5 bg-white border-2 border-sky-200 rounded-xl text-xs text-slate-900 font-medium focus:outline-none focus:ring-2 focus:ring-sky-500 shadow-2xs"
                        />
                        <button
                          type="button"
                          onClick={handleAddPlatformLink}
                          className="px-4 py-2.5 rounded-xl bg-sky-600 hover:bg-sky-500 text-white text-xs font-bold whitespace-nowrap cursor-pointer shadow-md shadow-sky-600/20 shrink-0 transition-all"
                        >
                          + Attach Link
                        </button>
                      </div>
                ...
                {/* Attached Links Preview List with Official Branded Logos (Task 3) */}
                <div className="space-y-2">
                  <span className="text-[11px] font-extrabold uppercase tracking-wider text-slate-500 block">
                    Student Resource Links Preview (Formatted Cards):
                  </span>
                  <div className="grid grid-cols-1 gap-3.5">
                    {attachedLinks.map((link) => {
                      const cfg = PLATFORM_CONFIG[link.platform] || PLATFORM_CONFIG.web;
                      return (
                        <div
                          key={link.id}
                          className="p-4 rounded-2xl bg-white border-2 border-sky-200/90 hover:border-sky-400 hover:shadow-md transition-all flex items-start justify-between gap-3 group"
                        >
                          <div className="flex items-start gap-3.5 min-w-0 flex-1">
                            <div className="w-11 h-11 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-center p-1.5 shrink-0 shadow-2xs">
                              {cfg.logoUrl ? (
                            ...
                            <div className="min-w-0 flex-1 space-y-1">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-[10px] font-bold text-emerald-700 flex items-center gap-1">
                                  <CheckCircle2 className="w-3 h-3 text-emerald-600" /> Verified Link
                                </span>
                              </div>
                              <h5 className="text-xs font-bold text-slate-900 leading-snug truncate group-hover:text-sky-700 transition-colors">
                                {link.title}
                              </h5>
                              <a
                                href={link.url}
                                target="_blank"
                                rel="noreferrer"
                                className="text-[11px] text-sky-600 font-semibold hover:text-sky-800 hover:underline truncate flex items-center gap-1 pt-0.5"
                              >
          ...
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="p-4 rounded-2xl bg-white border-2 border-sky-200/90 shadow-sm space-y-1 hover:border-sky-300 transition-all">
                ...
                <div className="text-2xl font-black text-slate-900 tracking-tight">{adminResources.length}</div>
