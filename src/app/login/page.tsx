  ...
  const handleGoogleAuth = async () => {
    ...
    try {
      ...
      if (err?.isCancelled || err?.code === 'auth/popup-closed-by-user') {
        ...
        setAuthNotice({
          type: 'blocked',
          message: 'Google popup was blocked by your browser. Please allow popups for localhost:3000 to sign in.',
        });
      } else {
        console.warn('Google Auth notice:', err?.message || err);
        setAuthNotice({
          type: 'error',
          message: err?.message || 'Google authentication could not be completed. Please try again.',
        });
      }
    } finally {
      setIsGoogleLoading(false);
